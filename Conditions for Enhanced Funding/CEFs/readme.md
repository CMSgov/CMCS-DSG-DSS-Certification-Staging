...
| **Description** | Metric data will be sourced from the POA&M(s), reported quarterly in <strong><em><font color="red">[list the months it will be reported in by the state here]</font></em></strong> on the Metric Values tab (using three measure counts), and sent to CMS, with each module’s ORW, quarterly along with the latest POA&M(s). |
